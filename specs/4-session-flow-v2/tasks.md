...
- [x] T001 🔴 Reescrever/criar as fixtures do fluxo novo em `fixtures/engine/`:
...
- [x] T002 🟢 `PreparePhase` + inserção antes de cada work em `src/engine/phases.ts`
- [x] T003 🟢 Motor TS em `src/engine/engine.ts`: `restDeadline`, rest auto-avança
...
- [x] T004 🟢 Motor Swift em lockstep: `watch/TapNextEngine/Sources/TapNextEngine/Models.swift`,
...
- [x] T005 🔴 Edge cases em `src/engine/__tests__/engine.test.ts`: pausa durante
...
- [x] T006 🟢 Cobrir os edges no TS e no Swift (mesmas fixtures/testes espelhados)

## Fase 2 — Domínio & persistência
> Objetivo: erro de importação com linha/coluna; retomada segura

- [x] T007 🔴 Teste em `src/domain/__tests__/workout.test.ts`: `parseWorkout` de
      texto com `"sets": 0` retorna erro com caminho `exercises[1].sets` + linha/coluna
- [x] T008 🟢 Localizador linha/coluna em `src/domain/workout.ts` (scanner do texto
      colado, sem dependência) + chaves i18n de erro em `src/i18n/pt-BR.json` e `en.json`
- [x] T009 Bump da versão do snapshot de sessão ativa em `src/data/activeSession.ts`
...
- [x] T010 Chaves i18n da sessão v2 em `src/i18n/pt-BR.json` e `src/i18n/en.json`
...
- [x] T011 [P] `src/ui/WheelPicker.tsx` — roda de rolagem com snap, números
      tabulares, escala de opacidade do protótipo
- [x] T012 [P] `src/ui/SegmentedProgress.tsx` e `src/ui/SetDots.tsx` — barra por
      exercício e pontos de série
- [x] T013 Refatorar `src/screens/SessionScreen.tsx` para a anatomia única:
...
- [x] T014 `src/screens/SummaryScreen.tsx` nova + rota `Summary` em
      `src/navigation/types.ts` — métricas, séries por exercício, ajustes em azul,
      CONCLUIR (`summary-done`)
- [x] T015 Ligar `src/session/SessionProvider.tsx` ao fluxo novo (tick dispara
      auto-abertura, countdown, navegação para Summary ao concluir)
- [x] T016 Flow E2E `e2e/flows/02-session-happy-path.yaml` atualizado para o
      fluxo Preparação-first até o Resumo

## Fase 4 — Sons por evento + importação com erro inline
> Critério de aceite: CA03, CA04, CA06

- [x] T017 Gerar assets curtos distintos em `assets/sounds/` (script) e mapear
      evento→som em `src/services/alerts.ts` (contagem, vai, fim isometria,
      início/fim descanso, sessão concluída)
- [x] T018 Erro inline no preview de `src/screens/ImportScreen.tsx` (destaque do
      trecho + cartão campo · linha · coluna; Importar desabilitado) — preservar
      testID `paste-button`
- [x] T019 Remover o botão "+" da lista se existir em `src/screens/HomeScreen.tsx`
- [x] T020 Flow E2E `e2e/flows/01-import-workout.yaml` atualizado (erro com
...
- [x] T021 `watch/TapNextWatch/Session/SessionViewModel.swift`: fase prepare,
      countdown, overtime, override pela coroa (reps/kg/tempo)
- [x] T022 `watch/TapNextWatch/Views/SessionView.swift`: palcos Preparação
      (valores + coroa), Execução reps/tempo, Descanso em anel; rótulo âmbar;
      hápticos por evento
- [x] T023 Strings em `watch/TapNextWatch/Resources/{en,pt-BR}.lproj/Localizable.strings`
